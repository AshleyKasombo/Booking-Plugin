...
// Message functionality for bookking module
 
class bookking_messenger {
    /**
     ...
     *
     */
    public static function compile_mail_template($template, $format, $parameters, $module = 'bookking', $lang = null) {
        $params = array ();
     ...
     * @param bookking_slot $slot The slot data as an MVC object, may be null
     ...
     */
    public static function get_bookking_variables(bookking_instance $bookking,  $slot,
        ...
        if ($bookking) {
            $vars['MODULE']     = format_string($bookking->name);
        ...
        if ($teacher) {
            ...
            $vars['ATTENDANT_URL'] = $CFG->wwwroot.'/user/view.php?id='.$teacher->id.'&course='.$bookking->course;
        }
        if ($student) {
            $vars['ATTENDEE']     = fullname($student);
            $vars['ATTENDEE_URL'] = $CFG->wwwroot.'/user/view.php?id='.$student->id.'&course='.$bookking->course;
        }
    ...
    /**
     * Send a notification message about a bookking slot.
     *
     * @param bookking_slot $slot the slot that the notification relates to
     ...
     */
    public static function send_slot_notification(bookking_slot $slot, $messagename, $template,
                                                  stdClass $sender, stdClass $recipient,
                                                  stdClass $teacher, stdClass $student, stdClass $course) {
        $vars = self::get_bookking_variables($slot->get_bookking(), $slot, $teacher, $student, $course, $recipient);
        self::send_message_from_template('mod_bookking', $messagename, 1, $sender, $recipient, $course, $template, $vars);
    }
