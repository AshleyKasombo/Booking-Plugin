YUI.add('moodle-mod_bookking-saveseen', function (Y, NAME) {
...
MOD.save_status = function(cmid, appid, newseen, spinner) {
    ...
    Y.io(M.cfg.wwwroot + '/mod/bookking/ajax.php', {
...
MOD.init = function(cmid) {
	Y.all(SELECTORS.CHECKBOXES).each( function(box) {
		box.on('change', function(e) {
			...
			M.mod_bookking.saveseen.save_status(cmid, box.get('value'), box.get('checked'), spinner);
		})
	});
};
