...
/**
 * A class for representing a bookking instance.
 *
 */
...
/**
 * A class for representing a bookking instance, as an MVC model.
 *
 */
class bookking_instance extends mvc_record_model {

    /**
     * @var stdClass course module record for this bookking
     */
    protected $cm = null;

    /**
     * @var stdClass course record for this bookking
     */
    protected $courserec = null;

    /**
     * @var context_module context record of this bookking
     */
    protected $context = null;

    /**
     * @var effective group mode of this bookking
     */
    protected $groupmode;

    /**
     * @var mvc_child_list list of slots in this bookking
     */
    protected $slots;
    ...
    protected function __construct() {
        parent::__construct();
        $this->slots = new mvc_child_list($this, 'bookking_slots', 'bookkingid',
            new bookking_slot_factory($this));
    }

    /**
     * Create a bookking instance from the database.
     *
     * @param int $id module id of the bookking
     * @return bookking_instance
     */
    public static function load_by_id($id) {
        global $DB;
        $cm = get_coursemodule_from_instance('bookking', $id, 0, false, MUST_EXIST);
     ...
     * Create a bookking instance from the database.
     *
     * @param int $cmid course module id of the bookking
     * @return bookking_instance
     */
    public static function load_by_coursemodule_id($cmid) {
        global $DB;
        $cm = get_coursemodule_from_id('bookking', $cmid, 0, false, MUST_EXIST);
        return self::load_from_record($cm->instance, $cm);
    }

    /**
     * Create a bookking instance from an already loaded record.
     *
     * @param int $id the module id of the bookking
     * @param stdClass $coursemodule course module record
     * @return bookking_instance
     */
    protected static function load_from_record($id, stdClass $coursemodule) {
        $bookking = new bookking_instance();
        $bookking->load($id);
        $bookking->cm = $coursemodule;
        $bookking->groupmode = groups_get_activity_groupmode($coursemodule);
        return $bookking;
    }
    ...
    public function delete() {
        ...
        bookking_grade_item_delete($this);
        parent::delete();
    }

    /**
     * Delete all slots from this bookking.
     */
    public function delete_all_slots() {
        $this->slots->delete_children();
        bookking_grade_item_update($this, 'reset');
    }

    /**
     * Retrieve the course module id of this bookking
     *
     * @return int
     */
    ...
    /**
     * Retrieve the course module record of this bookking
     *
     * @return stdClass
     */
    ...
    /**
     * Retrieve the course id of this bookking
     *
     * @return int
     */
    ...
    /**
     * Retrieve the course record of this bookking
     *
     * @return stdClass
     */
    ...
    /**
     * Retrieve the activity module context of this bookking
     *
    ...
    /**
     * Return the last modification date (as stored in database) for this bookking instance.
     *
     * @return int
     */
    ...
    /**
     * Retrieve the name of this bookking
     *
    ...
    /**
     * Retrieve the intro of this bookking
     *
     * @param bool $applyfilters whether to apply filters so that the output is printable
     * @return string
     */
    ...
    /**
     * Retrieve the name for "teacher" in the context of this bookking
     *
    ...
    public function get_teacher_name() {
        ...
        if (empty($name)) {
            $name = get_string('teacher', 'bookking');
        }
        return $name;
    }
    ...
    public function is_individual_scheduling_enabled() {
        if ($this->is_group_scheduling_enabled()) {
            return (bool) get_config('mod_bookking', 'mixindivgroup');
        } else {
            return true;
        }
    }
    ...
    /**
     * get the last location of a certain teacher in this bookking
     *
     * @param stdClass $user
     * @uses $DB
    ...
    public function get_last_location($user) {
        ...
        $select = 'bookkingid = :bookkingid AND teacherid = :teacherid ORDER BY timemodified DESC LIMIT 1';
        $params = array('bookkingid' => $this->data->id, 'teacherid' => $user->id);
        $lastlocation = $DB->get_field('bookking_slots', 'appointmentlocation', $params, IGNORE_MULTIPLE);
        if (!$lastlocation) {
            $lastlocation = '';
        }
        return $lastlocation;
    ...
    /**
     * Whether this bookking uses "appointment notes" visible to teachers and students
    ...
    /**
     * Whether this bookking uses "teacher notes" visible to teachers only
     * @return bool whether appointment notes are used
     */
    ...
    /**
     * Whether this bookking uses booking forms at all
    ...
    /**
     * Whether this bookking has booking instructions
    ...
    /**
     * Whether this bookking uses "student notes" filled by students at booking time
    ...
    /**
     * Whether this bookking uses student file uploads at booking time
    ...
    /**
     * Whether this bookking uses any data entered by the student at booking time
    ...
    /**
     * Whether this bookking uses captchas at booking time
    ...
    /**
     * Checks whether this bookking allows a student (in principle) to book several slots at a time
    ...
    /**
     * Checks whether this bookking allows unlimited bookings per student.
     * @return bool
     */
    ...
    /**
     * Checks whether this bookking uses grading at all.
     * @return boolean
     */
     ...
     *
     * @param int $bookkingid id of bookking
    ...
    public function get_user_grades($userid=0) {
        ...
        $sql = 'SELECT a.id, a.studentid, a.grade '.
               'FROM {bookking_slots} s JOIN {bookking_appointment} a ON s.id = a.slotid '.
               'WHERE s.bookkingid = :sid AND a.grade IS NOT NULL'.$usersql;
        ...
        if ($this->scale > 0) { // Grading numerically.
            ...
            foreach ($gradesums as $student => $gradeset) {
                switch ($this->gradingstrategy) {
                    case BOOKKING_MAX_GRADE:
                        $finalgrades[$student]->rawgrade = $gradeset->max;
                        break;
                    case BOOKKING_MEAN_GRADE:
    ...
    public function get_gradebook_info($studentid) {
        ...
        $gradinginfo = grade_get_grades($this->courseid, 'mod', 'bookking', $this->id, $studentid);
    ...
    protected function fetch_slots($wherecond, $havingcond, array $params, $limitfrom='', $limitnum='', $orderby='s.id') {
        global $DB;
        $select = 'SELECT s.* FROM {bookking_slots} s';

        $where = 'WHERE bookkingid = :bookkingid';
        if ($wherecond) {
            $where .= ' AND ('.$wherecond.')';
        }
        $params['bookkingid'] = $this->data->id;
    ...
    /**
     * Count a list of slots (for this bookking) in the database
     *
     * @param string $wherecond WHERE condition
     * @param array $params parameters for DB query
     * @return int
    ...
    protected function count_slots($wherecond, array $params) {
        global $DB;
        $select = 'SELECT COUNT(*) FROM {bookking_slots} s';
    ...
    protected function appointment_count_query() {
        return "(SELECT COUNT(a.id) FROM {bookking_appointment} a WHERE a.slotid = s.id)";
    }
    ...
    protected function student_in_slot_condition(&$params, $studentid, $mustbeattended, $mustbeunattended) {
        $cond = 'EXISTS (SELECT 1 FROM {bookking_appointment} a WHERE a.studentid = :studentid'.
    ...
    public function get_slot($id) {
        ...
        $slotdata = $DB->get_record('bookking_slots', array('id' => $id, 'bookkingid' => $this->id), '*', MUST_EXIST);
        $slot = new bookking_slot($this);
    ...
    /**
     * Retrieve a list of all slots in this bookking
     *
     * @return bookking_slot[]
     */
    ...
    /**
     * Retrieve the number of slots in the bookking
     *
     * @return int
     */
    ...
    /**
     * Does htis bookking have a slot where a certain student is booked?
     *
    ...
    /**
     * Does this bookking contain any slots where a certain group has booked?
     *
    ...
    public function has_slots_booked_for_group($groupid, $mustbeattended = false, $mustbeunattended = false) {
        ...
        $sql = "SELECT COUNT(*)
                  ...
                  JOIN {bookking_appointment} a ON a.slotid = s.id
                  JOIN {groups_members} gm ON a.studentid = gm.userid
                 WHERE s.bookkingid = :bookkingid
                       AND gm.groupid = :groupid
                       $attendcond";
        $params = array('bookkingid' => $this->id, 'groupid' => $groupid);
     ...
     * @return bookking_slot[] list of unused slots
     */
    ...
    function get_conflicts($starttime, $endtime, $teacher = 0, $student = 0,
                           $others = BOOKKING_SELF, $excludeslot = 0) {
        global $DB;
        ...
        switch ($others) {
            case BOOKKING_SELF:
                $bookkingscope = "sl.bookkingid = :bookkingid AND ";
                $params['bookkingid'] = $this->id;
                break;
        ...
        $studentjoin = ($student != 0) ? "JOIN {bookking_appointment} a ON a.slotid = sl.id AND a.studentid = :studentid " : '';
        ...
        $sql = "SELECT sl.*,
                       ...
                       (SELECT COUNT(*) FROM {bookking_appointment} ac WHERE sl.id = ac.slotid) AS numstudents
                  FROM {bookking_slots} sl
                       $studentjoin
                  JOIN {bookking} s ON sl.bookkingid = s.id
                  JOIN {course} c ON c.id = s.course
                 WHERE $slotscope $bookkingscope $teacherscope $timeclause
     ...
     *
     * @return mixed List of (bookking_slot, bookking_appointment)
     */
    public function get_slot_appointment($appointmentid) {
        global $DB;

        $appointrec = $DB->get_record('bookking_appointment', array('id' => $appointmentid), '*', MUST_EXIST);
        $slotrec = $DB->get_record('bookking_slots', array('id' => $appointrec->slotid), '*', MUST_EXIST);

        $slot = new bookking_slot($this);
        $slot->load_record($slotrec);
        $appointment = new bookking_appointment($slot);
     ...
     *
     * @param int $studentid
     * @return array of bookking_appointment objects
     */
    public function get_appointments_for_student($studentid) {
        ...
        $sql = "SELECT s.*, a.id as appointmentid
                  FROM {bookking_slots} s, {bookking_appointment} a
                 WHERE s.bookkingid = :bookkingid
                       AND s.id = a.slotid
                       AND a.studentid = :studid
              ORDER BY s.starttime";
        $params = array('bookkingid' => $this->id, 'studid' => $studentid);
        ...
        foreach ($slotrecs as $rec) {
            ...
            $appointrec = $DB->get_record('bookking_appointment', array('id' => $rec->appointmentid), '*', MUST_EXIST);
            $appointment = new bookking_appointment($slot);
            $appointment->load_record($appointrec);
    ...
    /**
     * Create a new slot relating to this bookking.
     *
     * @return bookking_slot
     */
    ...
    public function count_bookable_appointments($studentid, $includechangeable = true) {
        ...
        $sql = 'SELECT COUNT(*) FROM {bookking_slots} s'
              .' JOIN {bookking_appointment} a ON s.id = a.slotid'
              .' WHERE s.bookkingid = :bookkingid AND a.studentid=:studentid';
        if ($this->bookkingmode == 'onetime') {
        ...
        $params = array('bookkingid' => $this->id, 'studentid' => $studentid, 'cutofftime' => time() + $this->guardtime);
    ...
    /**
     * Get list of teachers that have slots in this bookking
     *
     * @return stdClass[]
     */
    ...
    protected function get_available_users($capability, $groupids = 0) {
        ...
        if (is_integer($groupids)) {
            ...
            foreach ($groupids as $groupid) {
                $groupusers = get_enrolled_users($this->get_context(), 'mod/bookking:appoint', $groupid,
                                                 'u.*', null, 0, 0, true);
    ...
    public function get_available_students($groupids = 0) {
        ...
        return $this->get_available_users('mod/bookking:appoint', $groupids);
    }
    ...
    public function get_available_teachers($groupids = 0) {
        ...
        return $this->get_available_users('mod/bookking:attend', $groupids);
    }

    /**
     * Checks whether there are any possible teachers for the bookking
     *
     ...
     *            another appointment. This matters onyl in bookkings where students can make
    ...
    public function delete_appointment($appointmentid) {
        ...
        if (!$oldrecord = $DB->get_record('bookking_appointment', array('id' => $appointmentid))) {
            return;
        }
     ...
     * This applies to all bookkings in the system.
     *
     * @uses $CFG
     * @uses $DB
     */
    public static function free_late_unused_slots() {
        ...
        $sql = "SELECT DISTINCT s.id
                           FROM {bookking_slots} s
                LEFT OUTER JOIN {bookking_appointment} a ON s.id = a.slotid
        ...
        if ($todelete) {
            ...
            $DB->delete_records_select('bookking_slots', " id $usql ", $params);
        }
    }
