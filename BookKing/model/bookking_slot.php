...
/**
 * A class for representing a bookking slot.
 *
 */
 ...
 * A class for representing a bookking slot.
 */
class bookking_slot extends mvc_child_record_model {
    ...
    protected function get_table() {
        return 'bookking_slots';
    }

    /**
     * Create a new slot in a specific bookking
     *
    ...
    public function __construct(bookking_instance $bookking) {
        ...
        $this->data->id = 0;
        $this->set_parent($bookking);
        $this->data->bookkingid = $bookking->get_id();
        $this->appointments = new mvc_child_list($this, 'bookking_appointment', 'slotid',
                        new bookking_appointment_factory($this));
    }

    /**
     * Create a bookking slot from the database.
     */
    public static function load_by_id($id, bookking_instance $bookking) {
        $slot = new bookking_slot($bookking);
        $slot->load($id);
        return $slot;
    }
     ...
     *
     * @param bookking_appointment $template appointment from which the data will be read
     */
    public function distribute_appointment_data(bookking_appointment $template) {
        $bookking = $this->get_bookking();
    ...
    private function distribute_file_area($area, $sourceid, $targetid) {
        ...
        $fs = get_file_storage();
        $component = 'mod_bookking';
        $ctxid = $this->get_bookking()->context->id;
     ...
     *
     * @return bookking_instance the bookking
     */
     ...
     * @return bookking_appointment the appointment for the specified student
     */
     ...
     *
     * @param bookking_appointment $app
     */
    ...
    public function delete() {
        ...
        $this->clear_calendar();
        $fs = get_file_storage();
        $fs->delete_area_files($this->get_bookking()->get_context()->id, 'mod_bookking', 'slotnote', $this->get_id());
        parent::delete();
    }
     ...
     * then, the id of the bookking slot that it belongs to.
    ...
    */
    ...
    private function update_calendar() {
        ...
        $bookkingname = $bookking->get_name(true);
        $bookkingdescription = $bookking->get_intro();

        $slotid = $this->get_id();
        $courseid = $bookking->get_courseid();

        $baseevent = new stdClass();
        $baseevent->description = "$bookkingname<br/><br/>$bookkingdescription";
        $baseevent->format = 1;
        $baseevent->modulename = 'bookking';
        ...
        $studenteventname = get_string('meetingwith', 'bookking').' '.$bookking->get_teacher_name().', '.fullname($teacher);
        ...
        if (count($studentids) > 0) {
            ...
            if (count($studentids) > 1) {
                $teachereventname = get_string('meetingwithplural', 'bookking').' '.
                                get_string('students', 'bookking').', '.implode(', ', $studentnames);
            } else {
                $teachereventname = get_string('meetingwith', 'bookking').' '.
                                get_string('student', 'bookking').', '.$studentnames[0];
            }
    ...
    private function update_calendar_events($eventtype, array $userids, stdClass $eventdata) {
        ...
        $existingevents = $DB->get_records('event', array('modulename' => 'bookking', 'eventtype' => $eventtype));
        $handledevents = array();
        $handledusers = array();
...
/**
 * A factory class for bookking slots.
 *
 */
class bookking_slot_factory extends mvc_child_model_factory {
    public function create_child(mvc_record_model $parent) {
        return new bookking_slot($parent);
    }
}
